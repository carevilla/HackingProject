...
HOST_IMAGE_CONFIG = {
    ...
    "gamma": {
        ...
        "served_path": "ops/leaked_gamma.jpg",
        "color": "#C8B6FF",
    ...
    "archive": {
        ...
        "served_path": "shared/leaked_archive.jpg",
        "color": "#9EC5FE",
...
def main() -> None:
    ...
    for host, config in HOST_IMAGE_CONFIG.items():
        ...
        image_path = host_dir / config["served_path"]
        image_path.parent.mkdir(parents=True, exist_ok=True)
        _create_image(image_path, config["color"], config["lines"], config["exif"])
        _write_index(host_dir, host, config["served_path"], config["port"])
...
def _write_index(host_dir: Path, host: str, served_path: str, port: int) -> None:
    hint_line = f"<p><a href=\"{served_path}\">{served_path}</a></p>"
    if "/" in served_path:
        top_dir = served_path.split("/", 1)[0]
        hint_line = "\n".join(
            [
                f"<p>Hint: interesting files might live under <a href=\"{top_dir}/\">/{top_dir}/</a></p>",
                f"<p><a href=\"{served_path}\">Direct file link</a></p>",
            ]
        )
    ...
    index_path.write_text(
        "\n".join(
            [
                ...
                f"<p>Host endpoint: http://127.0.0.1:{port}</p>",
                hint_line,
                "</body>",
                "</html>",
            ]
